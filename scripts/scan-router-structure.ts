...
function scanDirectory(dir: string, baseRoute: string = ''): Node[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const childNodes = scanDirectory(fullPath, isRouteGroup ? baseRoute : path.join(baseRoute, entry.name))
      ...
      nodes.push({
        ...
        name: entry.name,
        children: childNodes,
        status: isRouteGroup ? 'group' : (childNodes.some(child => child.route) ? 'active' : 'default')
      })
...
function scanRouterStructure() {
  ...
  if (!fs.existsSync(appDir)) {
    console.error('The "app" directory does not exist in the current working directory.')
    process.exit(1)
  }
